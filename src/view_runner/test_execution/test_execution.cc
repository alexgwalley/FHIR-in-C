...
ConvertSelect(Arena *arena, FHIR_VERSION::ViewDefinition_Select *select)
{
 ...
 if (select->_forEach.has_value)
 {
  ...
  {
   result->error = ViewDefinitionValidationError::InvalidFhirPath;
  }
...
ViewElem_ChildError(ViewElem* view)
{
 if (view->error != ViewDefinitionValidationError::Unknown) return view->error;
 if(view->type == ViewElemType::Select)
 {
   for (ViewElem *node = view->column_first; node; node = node->next)
   { 
    ViewDefinitionValidationError err = ViewElem_ChildError(node);
    if (err != ViewDefinitionValidationError::Unknown)
    {
     return err;
    }
   }
   ...
   {
    ViewDefinitionValidationError err = ViewElem_ChildError(v);
    if (err != ViewDefinitionValidationError::Unknown)
    {
     return err;
    }
 ...
 return ViewDefinitionValidationError::Unknown;
}
...
{
 Arena *test_arena = ArenaAlloc(Megabytes(2));
 ...
 for (int i = 0; i < col.test_count; i++)
 {
  ...
  try
  {
   table = ExecuteViewDefinition(test_arena, test.vd, &res_provider);
  }
...
//  Str8Lit("C:\\Users\\awalley\\Code\\sql-on-fhir-v2\\tests\\where.json")
 ...
 for (int tidx = 0; tidx < ArrayCount(tests); tidx++)
 {
  ...
  NullableBoolean nb = {};
  ViewDefinitionValidationError deserialization_error = ViewDefinitionValidationError::Unknown;
  FP_TestCollection test_col = DeserializeTestCollection(temp.arena, tests[tidx]);
  for (int i = 0; i < test_col.test_count; i++)
  {
   native_fhir::ViewDefinition vd = test_col.tests[i].vd;
   for (ViewElem* elem = vd.first; elem; elem = elem->next)
   {
    ViewDefinitionValidationError err = ViewElem_ChildError(elem);
    if (err != ViewDefinitionValidationError::Unknown)
    {
     nb.has_value = true;
     nb.value = test_col.tests[i].expect_error;
    }
   }
  }
  B32 passed = false;
  ...
  {
   passed = ExecuteTestCollection(test_col);
  }
