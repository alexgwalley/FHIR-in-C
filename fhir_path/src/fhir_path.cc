namespace native_fhir
{
	namespace nf_fhir_r4
	{
		...
		Number
		ParseNumber(U8 *start, U8* end_ptr, size_t *offset)
		{
			Number ret = { 0 };
		...
		GetNextPiece(Tokenizer *tokenizer, GetTokenFlags flags)
		{
			...
			String8 tok_str = Str8(tokenizer->pos, tokenizer->max_pos - tokenizer->pos);
			tok_str = Str8SkipWhitespace(tok_str);
			tokenizer->pos = tok_str.str;
			...
			if (!is_binary_op && ret->type == Piece_Unknown)
			{
				...
				if (CharIsDigit(*tokenizer->pos))
				{
					...
					String8 tok_str = Str8(tokenizer->pos + offset, tokenizer->max_pos - tokenizer->pos);
					tok_str = Str8SkipWhitespace(tok_str);
					...
					{
						String8 unit = { 0 };
						U8* pos = tok_str.str + 1;
						unit.str = pos;
						while (*pos != '\'' && pos < tokenizer->max_pos)
						{
							pos++;
						}
						if (tokenizer->pos >= tokenizer->max_pos)
						{
							Assert(false);
						}
						// NOTE(agw): eat ending '
						...
						offset += pos - (tokenizer->pos + offset);
				...
				else if (*tokenizer->pos == '\'')
				{
					...
					ret->value.str.str = pos;
					while (*pos != '\'' && pos < tokenizer->max_pos)
					{
						pos++;
						offset++;
					}
					ret->value.str.size = offset;
					ret->value.str.has_value = TRUE;
